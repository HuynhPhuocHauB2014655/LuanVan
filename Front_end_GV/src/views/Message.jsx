                            ...
                            <div className="grid grid-rows-1 grid-flow-col h-full">
                                <div className="relative h-full w-full">
                                    ...
                                    <div className="h-[76%] overflow-y-auto flex-col-reverse w-full space-y-7">
                                        {messages?.filter(item => (item.NguoiGui == `${userName}-${info.TenGV}` && item.NguoiNhan == selectedGroup.id) || item.NguoiNhan == `${userName}-${info.TenGV}`).map((tn) => (
                                            <div key={tn.id} className={` whitespace-pre-wrap rounded-md shadow-md w-[40%] border-2 px-2 py-3 mx-2 flex ${tn.NguoiGui !== `${userName}-${info.TenGV}` ? '' : 'ml-auto justify-end'}`}>{tn.TinNhan}</div>
                                        ))}
